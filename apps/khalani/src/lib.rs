...
5. If a returned step is `stage_tx`, call `stage_tx` with the exact hinted JSON arguments verbatim. Do not wrap them, rename fields, or rebuild the `data.raw` object yourself. Then run `simulate_batch`, then `commit_txs` before advancing to any Khalani callback-dependent step.
...
- If the route hint already contains `to`, `description`, `data.raw`, `value`, or `gas_limit`, copy those fields directly into `stage_tx` and do not transform their shape.
